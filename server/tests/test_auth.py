...
async def test_full_auth_flow_mobile_and_web():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        resend_res = await client.post("/auth/resend-verification", json={"email": "user@example.com"})
        assert resend_res.status_code == 200

        # 3. Attempt Login Before Verification -> 403 Forbidden
        login_payload = {
            "email": "user@example.com",
            "password": "SecurePassword123!",
        }
        ...
        # 5. Mobile Client Login -> Tokens returned in JSON body
        ...
        # 6. Access Protected Endpoint /auth/me with Bearer token
        ...
        # 7. Web Client Login -> Tokens returned in HttpOnly cookies
        ...
        # 8. Mobile Refresh Token Rotation
        ...
        # 9. Refresh Token REUSE ATTACK DETECTION: Reuse old mobile_refresh_token
        ...
        # 10. Invalidation check: The new refresh token should also be revoked due to family cancellation!
...
async def test_password_reset_flow():
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        # Register user and mark verified
        await client.post("/auth/register", json={"email": "reset@example.com", "password": "OldPassword123!"})

        async with TestAsyncSessionLocal() as db:
            user_stmt = select(User).where(User.email == "reset@example.com")
            user_res = await db.execute(user_stmt)
            user = user_res.scalar_one()
            user.is_verified = True
            await db.commit()
